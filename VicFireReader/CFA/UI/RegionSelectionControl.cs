#region Copyright

/*---------------------------------------------------------------------------
 * The contents of this file are subject to the Mozilla Public License
 * Version 1.1 (the "License"); you may not use this file except in compliance
 * with the License. You may obtain a copy of the License at
 * 
 * http://www.mozilla.org/MPL/
 * 
 * Software distributed under the License is distributed on an "AS IS"
 * basis, WITHOUT WARRANTY OF ANY KIND, either express or implied. See the
 * License for the specific language governing rights and limitations under 
 * the License.
 * 
 * The Initial Developer of the Original Code is Robert Smyth.
 * Portions created by Robert Smyth are Copyright (C) 2008.
 * 
 * All Rights Reserved.
 *---------------------------------------------------------------------------*/

#endregion //Copyright

using System;
using System.Windows.Forms;
using CFAReader.CFA.Regions;
using NDependencyInjection;


namespace CFAReader.CFA.UI
{
	public partial class RegionSelectionControl : UserControl, IRegionSelectionControl
	{
		private readonly ICfaRegions regions;

		public RegionSelectionControl()
		{
			InitializeComponent();
		}

		[InjectionConstructor]
		public RegionSelectionControl(ICfaRegions regions) : this()
		{
			this.regions = regions;
			regionComboBox.DataSource = regions;
		}

		private void regionComboBox_SelectedIndexChanged(object sender, EventArgs e)
		{
			regions.SelectedRegion = regionComboBox.SelectedItem as ICfaRegion;
		}

		private void RegionSelectionControl_Load(object sender, EventArgs e)
		{
			regionComboBox.SelectedItem = regions.SelectedRegion;
			regionComboBox.SelectedIndexChanged += regionComboBox_SelectedIndexChanged;
		}
	}
}